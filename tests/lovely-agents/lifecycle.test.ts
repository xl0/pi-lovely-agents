...
import { discardTask, reconcileParentTasks, stopOwnedTaskTree } from "../../extensions/lovely-agents/lifecycle.js"
import {
	acquireParentLease,
	ensureParentStorage,
...
describe("task lifecycle recovery", () => {
	...
	test("interrupts stale active work once with one notice, skipping residents and stable records", async () => {
		await withTempWorkspace(async workspace => {
			...
			const agent = await createTask(workspace.cwd, "parent", "a_11111111", "child", "running")
			const queued = await createTask(workspace.cwd, "parent", "a_50000001", "child-1", "queued")
			const interrupted = await createTask(workspace.cwd, "parent", "a_50000005", "child-5", "interrupted")
			...
			try {
				expect(await reconcileParentTasks(workspace.cwd, "parent")).toEqual({ interrupted: 3, diagnostics: [] })
				...
				expect(await readTaskMetadata(agent)).toMatchObject({
					metadata: {
						state: "interrupted",
						latestOutcome: "interrupted",
						activeRun: null,
				...
				expect(await readFile(agent.history, "utf8")).toContain("interrupted")
				expect(await readTaskMetadata(queued)).toMatchObject({ metadata: { state: "interrupted" } })
				expect(await readTaskMetadata(interrupted)).toMatchObject({ metadata: { state: "interrupted", notifications: [] } })
				...
				expect(await readTaskMetadata(agent)).toMatchObject({ metadata: { notifications: [{ type: "interruption" }] } })
			} finally {
				unbind()
	...
	test("recursively stops retained work and clears queued follow-ups", async () => {
		await withTempWorkspace(async workspace => {
			...
			const nested = await createTask(workspace.cwd, "child", "a_33333333", "grandchild", "queued")
			await stopOwnedTaskTree(workspace.cwd, "parent")
